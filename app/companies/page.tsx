...
export default function CompaniesPage() {
  ...
  if (error) {
    const isDatabaseError = error.includes('데이터베이스') || error.includes('DATABASE_URL') || error.includes('연결');
            ...
            {isDatabaseError && (
              <div className="mt-4 p-4 bg-white rounded-lg border border-red-200">
                <p className="text-sm text-gray-700 mb-3">
                  데이터베이스 연결 설정이 필요합니다. 다음 단계를 따라주세요:
                </p>
                <Link
                  href="/setup"
                  className="inline-block bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors mr-2"
                >
                  설정 가이드 보기
                </Link>
                <button
